...
C(1:66,1:66) = C(1:66,1:66) + C1; %Damping from the Basic Structure
...
t = h;
while t <= 2
    R1 = r01*cos(2*pi*W1*t); %Ground Excitation on the First Subsystem at Time Instance t
    R1d = -2*pi*W1*r01*sin(2*pi*W1*t); %Derivative of the Ground Excitation on the First Subsystem at Time Instance t
    R2 = r02*sin(2*pi*W2*t); %Ground Excitation on the Second Subsystem at Time Instance t
    R2d = 2*pi*W2*r02*cos(2*pi*W2*t); %Derivative of the Ground Excitation on the Second Subsystem at Time Instance t
    R3 = r03*sin(2*pi*W3*t); %Ground Excitation on the Third Subsystem at Time Instance t
    R3d = 2*pi*W3*r03*cos(2*pi*W3*t); %Derivative of the Ground Excitation on the Third Subsystem at Time Instance t
    R4 = r04*cos(2*pi*W4*t); %Ground Excitation on the Fourth Subsystem at Time Instance t
    R4d = -2*pi*W4*r04*sin(2*pi*W4*t); %Derivative of the Ground Excitation on the Fourth Subsystem at Time Instance t
    F = zeros(70,1); %Force Vector at Time Instance t
    F(67) = cw*R1d + kw*R1; %Excitation Force on the First Subsystem at Time Instance t
    F(68) = cw*R2d + kw*R2; %Excitation Force on the Second Subsystem at Time Instance t
    F(69) = cw*R3d + kw*R3; %Excitation Force on the Third Subsystem at Time Instance t
    F(70) = cw*R4d + kw*R4; %Excitation Force on the Fourth Subsystem at Time Instance t

    a(round(t/h)+1,:) = (A*(F - C*(v(round(t/h),:)' + h*(1-b)*a(round(t/h),:)') - K*(u(round(t/h),:)' + h*v(round(t/h),:)' + h^2*(1/2-alpha)*a(round(t/h),:)')))'; %Acceleration at each Time Instance t
    v(round(t/h)+1,:) = v(round(t/h),:) + h*((1-b)*a(round(t/h),:) + b*a(round(t/h)+1,:)); %Velocity at each Time Instance t
    u(round(t/h)+1,:) = u(round(t/h),:) + h*v(round(t/h),:) + h^2*((1/2-alpha)*a(round(t/h),:) + alpha*a(round(t/h)+1,:)); %Displacement at each Time Instance t
